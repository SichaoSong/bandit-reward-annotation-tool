...
- 前回保存したCSVを読み込むと、同じ動画フォルダーの提示順と作業履歴を復元可能
...
2. 前回の作業を続ける場合は「前回CSVから再開」または作業中の「CSVから作業再開」でCSVを選びます。
3. 動画読み込み後、作業モードで上部の「作業指示」を確認します。
4. 動画を再生し、1〜7で採点します。
5. 理由を50文字以上入力します。
6. 「次へ」でブラウザ内に保存して次の動画へ進みます。
7. 前の動画を修正したい場合は「前へ」で戻り、修正後に「次へ」または「保存」を押します。
8. CSVとしてローカル保存したいタイミングで「保存」を押します。最後の動画では「保存」で作業内容を確定します。

作業を翌日に再開する場合は、同じ動画フォルダーを読み込んでから「CSVから作業再開」で前回保存したCSVを選びます。CSV内の `video_id` と現在のフォルダー内動画を照合し、CSVに保存された `presentation_order` と評価済み内容を復元します。
